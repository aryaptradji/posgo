<x-layout class="mb-0">
    ...
    <form action="{{ route('expense.store') }}" method="POST" enctype="multipart/form-data" class="mt-10 rounded-xl grid grid-cols-2 gap-8"
        ...
        <div class="col-span-1 flex flex-col gap-4">
            {{-- Kategori --}}
            <x-dropdown class="mb-2" name="category" :items="['Operasional' => 'Operasional', 'Luar Operasional' => 'Luar Operasional']" :value="old('category', 'Pilih Salah Satu')">Kategori</x-dropdown>
            @error('category')
