<x-layout>
    <x-slot:title>Ubah Kasir</x-slot:title>
    <x-slot:header>
        <div class="flex mb-2 items-center gap-2 text-sm text-tertiary-title">
            <a href="{{ route('cashier.index') }}"
                class="font-semibold transition-all duration-300 hover:text-primary hover:scale-110 active:scale-90">Kasir</a>
    ...
    <form action="{{ route('cashier.update', $cashier) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="mt-10 grid grid-cols-2 gap-8"
            x-data="{
                name: '{{ old('name', $cashier->name) }}',
                email: '{{ old('email', $cashier->email) }}',
                phone: '{{ old('phone_number', $cashier->phone_number) }}',
                password: '{{ old('password', $cashier->plaintext_password) }}',
                nameError: '',
                emailError: '',
                phoneError: '',
                passwordError: '',
        ...
        >
            ...
            <div class="col-span-1">
                <x-textfield-image class="h-3/4" name="photo"
                    initial-image-url="{{ asset('storage/' . $cashier->photo) }}"
                    initial-file-name="{{ basename($cashier->photo) }}"
                    initialFileSize="{{ $sizeInKB }}">
                    Foto
                    <span class="ml-2 text-xs text-tertiary-400 font-semibold">(format .jpg/.jpeg, max. 2 mb)</span>
                </x-textfield-image>
                <div class="col-span-2 flex justify-center gap-6 mt-10">
                    ...
                    <x-button-sm type="submit"
                        class="w-fit px-7 text-primary bg-primary/20">Simpan</x-button-sm>
                </div>
            </div>
        </div>
    </form>
