<x-layout class="mb-0">
    ...
    <form action="{{ route('revenue.update', $revenue) }}" method="POST" enctype="multipart/form-data" class="mt-10 rounded-xl grid grid-cols-2 gap-8"
        x-data="{
        ...
        <div class="col-span-1 flex flex-col gap-4">
            {{-- Waktu --}}
            <x-textfield type="datetime-local" name="date" :value="old('date', $revenue->date)" class="focus-within:ring focus-within:ring-primary" classCont="mb-2">
                Waktu
            </x-textfield>

            {{-- Sumber --}}
            <x-dropdown-toggle class="mb-2" name="source" :items="array_merge($products, ['Penjualan Langsung', 'Refund Customer', 'Pendapatan Lainnya'])" :value="old('source', $revenue->source)">Sumber</x-dropdown-toggle>
            @error('source')
        ...
        <div class="col-span-1 flex flex-col gap-4">
            {{-- Kategori --}}
            <x-dropdown class="mb-2" name="category" :items="['Produk' => 'Produk', 'Luar Produk' => 'Luar Produk']" :value="old('category', $revenue->category)">Kategori</x-dropdown>
            @error('category')
                <x-inline-error-message class="mb-2 -mt-2" x-show="categoryServerError">{{ $message }}</x-inline-error-message>
            @enderror

            {{-- Total --}}
            <x-textfield-price class="focus:ring focus:ring-primary" name="total" :value="old('total', $revenue->total)">Total</x-textfield-price>
        </div>
        <div class="col-span-2 flex justify-center gap-6 mt-3">
            <x-button-sm class="w-fit px-7 text-black bg-btn-cancel">
                <a href="{{ route('revenue.index') }}">Batal</a>
            </x-button-sm>
