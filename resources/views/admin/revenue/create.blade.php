<x-layout class="mb-0">
    <x-slot:title>Buat Pemasukan</x-slot:title>
    <x-slot:header>
        <div class="flex mb-2 items-center gap-2 text-sm text-tertiary-title">
            <a href="{{ route('revenue.index') }}"
                class="font-semibold transition-all duration-300 hover:text-secondary-purple hover:scale-110 active:scale-90">Pemasukan</a>
    ...
    <form action="{{ route('revenue.store') }}" method="POST" enctype="multipart/form-data" class="mt-10 rounded-xl grid grid-cols-2 gap-8"
        x-data="{
        ...
        <div class="col-span-1 flex flex-col gap-4">
            ...
            {{-- Sumber --}}
            <x-dropdown-toggle class="mb-2" name="source" :items="array_merge($products, ['Penjualan Langsung', 'Komisi', 'Bonus Supplier'])" :value="old('source', 'Pilih Salah Satu')">Sumber</x-dropdown-toggle>
            @error('source')
        ...
        <div class="col-span-1 flex flex-col gap-4">
            {{-- Kategori --}}
            <x-dropdown class="mb-2" name="category" :items="['Produk' => 'Produk', 'Luar Produk' => 'Luar Produk']" :value="old('category', 'Pilih Salah Satu')">Kategori</x-dropdown>
            @error('category')
